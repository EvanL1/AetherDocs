...
This repository publishes unified English and Simplified Chinese documentation
...
Canonical production URL: `https://docs.aetheriot.ai`.

## Representations

- Browser requests receive the Astro + Starlight HTML site.
- A `.md` suffix or `Accept: text/markdown` receives the matching Markdown.
- `llms.txt` is the English agent index.
- `zh/llms.txt` is the Chinese agent index.
...
product sources are written at the root locale; Chinese sources are written
under `zh/`. The sync namespaces non-Edge routes by product, rewrites relative
...
pages use `https://docs.aetheriot.ai`, with the root path for English
...
2. Reject CJK characters in the root English locale and reject untranslated
   `/zh/` pages.
