...
| --- | --- | --- | --- | --- |
| 带有 alpha.3 使用方的 `v0.0.1` | `v0.1.0-alpha.3` | 当前未发布的 alpha.3 使用方 | 实验性集成基线 | 相同的完整使用方锁定、53 个精确导入、无待处理导入和 25 个共享固定结果 |
| `v0.0.1` 旧版 MQTT 路径 | 旧版线协议不需要 | 现有旧版接入 | 兼容性默认值 | 现有产品行为；CloudLink 不会静默重新解释旧 Topic |
