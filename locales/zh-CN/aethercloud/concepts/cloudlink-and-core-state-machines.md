...
实现了冻结域转换、受控发布/查询应用程序用例以及原子内存内容/签名/元数据/审核/发件箱适配器。上传/最终确定、生产对象存储和签名者策略、PostgreSQL 元数据、持久审计/发件箱、通道移动、生命周期 HTTP 和边缘工件交付仍在计划中。请参阅[工件注册表](/zh/aethercloud/concepts/artifact-registry) 了解确切的图层状态。
